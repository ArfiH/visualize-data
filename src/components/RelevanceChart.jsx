...
const RelevanceChart = () => {
  ...
  useEffect(() => {
    ...
    })).filter(d => d.category && d.relevance !== null); // Filter out invalid data

    // Group data by category and calculate mean relevance
    const groupedData = d3.rollups(
      parsedData,
      v => d3.mean(v, d => d.relevance),
      d => d.category
    ).map(([key, value]) => ({ category: key, meanRelevance: value }));
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(groupedData, d => d.meanRelevance)]).nice()
    ...
    svg.append("g")
      ...
      .attr("y", d => y(d.meanRelevance))
      .attr("height", d => y(0) - y(d.meanRelevance))
      .attr("width", x.bandwidth())
      .attr("fill", "steelblue")
      .append("title")  // Tooltip
      .text(d => `${category.charAt(0).toUpperCase() + category.slice(1)}: ${d.category}\nMean Relevance: ${d.meanRelevance.toFixed(2)}`);
